...
import { fetchAllCustomersAction, searchCustomerAction, sortCustomerByTotalAmountAction } from "src/features/customers/actions";
import formatCustomersList, { CustomerType } from "src/features/customers/utils/formatCustomersList";
...
export default function CustomersTable({
...
}:{customers:any[], currentPageNumber:number, totalPagesNumber:number, paginate:Function}) {

    const [customerState, setCustomerState] = useCustomerState()

    const tableHead = ['Customers Name', 'Customers Name', 'Phone Number & Email', 'Total Amount Traded', 'Status'];
    ...
    useEffect(()=> {
        const customerToDisplay:any = customerState.searchKeyword ?searchResult :customers
    ...
    const searchCustomers = (value:string, page:number)=> {
        ...
        .then((response:any)=> { 
            const formatted:CustomerType[] = formatCustomersList(response.customers);
    ...
    const downloadCSV = ()=> {
        ...
        .then((response:any)=> { 
            const formatted:any[] = formatCustomersCSVList(sortByDate(response.customers)!);

            const csvTableHead = ['Name', 'Email', 'Phone number', 'Amount traded', 'Registered', 'Last seen']
    ...
    const formatCustomersCSVList =(allCustomers:any[])=> {
        ...
        return allCustomers.map((customer:any) => {
            const phone =   customer.phoneNumber.toString().substr(0, 3) === '234'
                            ? customer.phoneNumber.toString().substr(3, customer.phoneNumber.length) 
        ...
        <div className={styles.customers_table_container}>
            ...
            
            {
                ...
                :   <Table
                        ...
                        goToPage={(newPageNumber:number)=> customerState.searchKeyword ?paginateSearchResult(newPageNumber) : customerState.sortType ?paginateSortResult(newPageNumber) :paginate(newPageNumber) }
