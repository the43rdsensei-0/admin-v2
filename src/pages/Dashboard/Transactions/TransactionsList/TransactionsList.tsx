...
export default function TransactionsList() {
    ...
    const [isTransactionLoading, setIsTransactionLoading] = useState(transactionState.list.length ?false :true);

    useEffect(()=> {
        if(!transactionState.searchKeyword && !transactionState.activeCategory) {
            socketConnection.emit(`fetch_all_transactions`, { pageNumber: transactionState.currentPageNumber, userId: userState.userData.id })
        }

        if(!transactionState.searchKeyword && transactionState.activeCategory === 'cryptocurrency') {
            if(transactionState.activeCSACryptoCategory === 'sell') {
                socketConnection.emit('leave_buy_crypto_transactions_room', { userId: userState.userData.id })
                socketConnection.emit('fetch_sell_crypto_transactions', { pageNumber: transactionState.currentPageNumber, userId: userState.userData.id })
            }

            if(transactionState.activeCSACryptoCategory === 'buy') {
                socketConnection.emit('leave_sell_crypto_transactions_room', {userId: userState.userData.id})
                socketConnection.emit('fetch_buy_crypto_transactions', { pageNumber: transactionState.currentPageNumber, userId: userState.userData.id })
            }
        }

        socketConnection.on('transactions', (response)=> {
            if( response.data?.currentTransactionPage === transactionState.currentPageNumber && 
                ...
                .catch((error)=> {
                    setTransactionState(state => {
                        return {
                            ...
                            status: "failed",
                            error: true,
                            message: "There was an error fetching transactions, please try again. If issue persists, contact support."
                        }
                    })
                })
            }
    ...
    }, [setTransactionState, transactionState.activeCSACryptoCategory, transactionState.activeCategory, transactionState.currentPageNumber, transactionState.searchKeyword, userState.userData.id])
    ...
    const fetchCatTransactions = (category:string)=> {
        setIsTransactionLoading(true)
        socketConnection.emit(category, { pageNumber: 1, userId: userState.userData.id })
    }
                    ...
                    :   <div>
...

                            {
                                ...
                                :   <TransactionsTable 
                                        ...
                                        fetchCategorizedTransactions={(category:string)=> fetchCatTransactions(category)}
                                        updateFilter={(filterOptions:FilterOptionsType)=> filterTransactions(filterOptions)}
                                    />
