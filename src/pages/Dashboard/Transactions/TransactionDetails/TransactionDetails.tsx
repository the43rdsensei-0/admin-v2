...
import {
  ...
  useTransactionState,
} from "../../../../features/transactions/atom";
...
} from "src/features/transactions/utils/formatTransaction";
...
} from "src/features/transactions/actions";
...
export default function TransactionDetails() {
  ...
  const [rejectTransactionState, setRejectTransactionState] =
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      socketConnection.emit("leave_transaction_room", {
        transactionId,
        userId: userState.userData.id,
      });
    };
  ...
  const [amountReceivedModel, setAmountReceivedModel] =
    useState<formFieldProps>({
      ...
      value:
        transactionState.details.txnType === "BUY"
          ...
          : transactionState.details.amountSent.inDollar,
      error: "",
      validated: false,
    });
  ...
  const [noteToCustomerModel, setNoteToCustomerModel] =
    useState<formFieldProps>({
      type: "text",
  ...
  const setInput = (
    ...
    if (model.name === "confirmed-rate") {
      ...
      const amountToUse = amountReceivedModel.validated
        ? amountReceivedModel.value
        : transactionState.details.amountSent.inDollar;
      calculateAmountToPay(model.value, amountToUse!);
    }

    if (model.name === "amount-received") {
      const rateToUse = confirmedRateModel.validated
        ? transactionState.details.rate.confirmed
        : transactionState.details.rate.sent;
  ...
  const calculateAmountToPay = (
    confirmedRate: number | string,
    amountConfirmed: string | number
  ) => {
    let amountToPayResult = 0;
    if (transactionState.details.txnType === "BUY") {
      amountToPayResult =
        parseFloat(amountConfirmed?.toString()!) /
        parseFloat(confirmedRate.toString());
    } else {
      amountToPayResult =
        parseFloat(amountConfirmed?.toString()!) *
        parseFloat(confirmedRate.toString());
  ...
  const reviewTransaction = () => {
    if (validateForm()) {
      const payload = {
        confirmedRate:
          parseFloat(confirmedRateModel.value?.toString()!) ||
          parseFloat(transactionState.details.rate.sent.toString()),
  ...
  const rejectReviewed = () => {
    ...
    socketConnection
      .emit(`reject_reviewed_transaction`, {
        transactionId,
        userId: userState.userData.id,
      })
            ...
            .catch((error: any) => {
              setRejectTransactionState((state) => {
                return {
                  ...
                  error: true,
                  message:
                    "There was an error rejecting transaction, try again. If issue persists, contact support.",
                };
              });
            })
            .finally(() => {
        ...
        <div className={styles.content_wrapper}>
          ...
          <div className={styles.content}>
            ...
            <div className={styles.content_section}>
              ...
              <div className={`${styles.content_details} ${styles.txn_id}`}>
                #{transactionState.details?.id}
              </div>
            </div>
              ...
              <div className={`${styles.content_details} ${styles.asset_info}`}>
                ...
                <div className={styles.text}>
                  <div className={styles.name}>
                    {transactionState.details?.assetCategory}
                  </div>
                  <div className={styles.subcategory}>
                    {transactionState.details?.assetSubcategory}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className={styles.content_section}>
              ...
              <div className={styles.content_details}>
                <div className={styles.txn_date}>
                  {formatDate(transactionState.details?.date)}
                </div>
                <div className={styles.time}>
                  {formatTime(transactionState.details?.date)}
                </div>
              </div>
            </div>
                ...
                <div className={styles.content_section}>
                  ...
                  <div className={styles.content_details}>
                    <div className={styles.amount_sent}>
                      {transactionState.details?.txnType === "BUY"
                        ? convertCurrency(
                            transactionState.details?.amountSent?.inNaira || 0,
                            "₦",
                            2
                          )
                        : convertCurrency(
                            transactionState.details?.amountSent?.inDollar || 0,
                            "$",
                            2
                          )}
                    </div>
                    {transactionState.details?.txnName === "cryptocurrency" ? (
                      <div className={styles.rate}>
                        rate:{" "}
                        {convertCurrency(
                          transactionState.details?.rate.sent || 0,
                          "₦",
                          2
                        )}
                        /$
                  ...
                  <div className={`${styles.content_details}`}>
                    ...
                    <div className={styles.rate}>
                      <div>
                        Confirmed rate:{" "}
                        {convertCurrency(
                          transactionState.details?.rate.confirmed || 0,
                          "₦",
                          2
                        )}
                        /$
                ...
                {transactionState.details?.txnType === "BUY" ? (
                  ...
                  <div className={styles.content_section}>
                    <div className={styles.label}>
                      Value in {transactionState.details?.assetCategory}
                    </div>
                    <div className={styles.content_details}>
                      <div className={styles.amount_sent_in_crypto}>
                        {transactionState.details?.amountSent?.inCrypto}
                      </div>
                      <div className={styles.rate}>
                        rate:{" "}
                        {convertCurrency(
            ...
            {transactionState.details.txnType === "BUY" ? (
              <div className={styles.content_section}>
                ...
                <div className={styles.bank_card}>
                  <div className={styles.name}>
                    {transactionState.details.bankUsed?.name}
                  </div>
                  <div className={styles.account_number_capsule}>
                    {transactionState.details.bankUsed?.number}
                  </div>
                </div>
              </div>
            ) : null}
            ...
            {transactionState.details.attendants.includes(
              userState.userData.id
            ) ||
            ["admin_org", "admin_ops", "admin_acct"].includes(
              userState.userData.role
            ) ? (
              transactionState.details.reviewed ? (
                <div className={styles.buttons}>
                  {transactionState.details.paidout ? null : (
                  ...
                  "admin_csa_crypto",
                  "admin_csa_card",
                ].includes(userState.userData.role) ? (
                <FormContainer extraStyle={styles.transaction_details}>
                  ...
                  <InputField
                    ...
                    value={
                      transactionState.details.txnType === "BUY"
                        ? transactionState.details.amountSent.inNaira
                        : amountReceivedModel.value ||
                          transactionState.details.amountSent.inDollar.toFixed(
                            2
                          )
                    }
                  ...
                  <AmountToPayCard
                    ...
                    amount={
                      transactionState.details.txnType === "BUY"
                        ? parseFloat(amountToPayModel.value?.toString()!) ||
                          transactionState.details.amountSent.inDollar
                        : parseFloat(amountToPayModel.value?.toString()!) ||
                          transactionState.details.amountSent.inNaira
                    }
                  ...
                  <TextField
                    ...
                    defaultValue={
                      noteToCustomerModel.defaultValue?.toString()! ||
                      transactionState.details?.note?.message
                    }
                    onInput={(textValue: string) =>
                      setInput(
                        textValue,
                        noteToCustomerModel,
                        setNoteToCustomerModel
                      )
                    }
                    onAttach={(attachmentImages: []) =>
                      setNoteAttachmentImages(attachmentImages)
                    }
              ...
              transactionState.details.customerAttendant !==
                userState.userData.id ? (
              <TransactionNoteCard
            ...
            <div className={styles.people_section}>
              <Card extraStyle={styles.attendant_details_card}>
                <Suspense fallback={<ComponentLoader />}>
                  {transactionState.details.reassigned ||
                  ...
                  {transactionState.details.reviewed ? (
                    <div className={styles.amount_to_pay}>
                      ...
                      <div className={styles.value}>
                        {convertCurrency(
                          transactionState.details?.amountToPay || 0,
                          "₦",
                          2
                        )}
                      </div>
