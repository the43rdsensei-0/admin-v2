...
export default function Login() {
    ...
    const LoginAuth = async ()=> {
       if(validateForm()) {
            ...
            .then((response:any)=> {
                ...
                if(['admin_acct', 'admin_csa_crypto', 'admin_csa_card'].includes(response?.data?.user?.role)) navigate({pathname: "/dashboard/transactions"})
    ...
    useEffect(()=> {
        if(authState.isSignedIn && userState.userData.id) {
            if(['admin_acct', 'admin_csa_crypto', 'admin_csa_card'].includes(userState.userData.role)) navigate({pathname: "/dashboard/transactions"})
            else navigate({pathname: "/dashboard"})
        }
    })
    ...
    return (
        <div className={styles.body}>
            ...
            <FormErrorModal 
                errorState={authState}
                setErrorState={setAuthState}
            />
            ...
            <div className={styles.form_container}>
                ...
                <FormContainer formHeading="Login To Admin" extraStyle={styles.form_wrapper}>
                    <InputField
                        label={EmailModel.label}
                        type={EmailModel.type}
                        onInput={(inputVal:string)=> setInput(inputVal, EmailModel, setEmailModel)}
                    ...
                    <PasswordField
                        ...
                        onInput={(inputVal:string)=> setInput(inputVal, PasswordModel, setPasswordModel)}
                    />
